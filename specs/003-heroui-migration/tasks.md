...
- [X] T017 [P] [US1] Migrate card containers in `frontend/src/features/admin/AdminOrderDetailPage.tsx` to HeroUI `Card` compound per Anatomy A.1 (existing HeroUI `Modal` stays — confirm/destroy actions in this page migrate to `AlertDialog` in US10 per FR-010c).
...
- [X] T023 [P] [US1] Migrate card containers in `frontend/src/features/orders/OrderDetailPage.tsx` to HeroUI `Card` compound per Anatomy A.1 (`Snippet` wrapper stays — Exception 2).
...
- [X] T026 [US1] Run `npm --prefix frontend run build` — must pass; record bundle-size delta vs. baseline.
- [X] T027 [US1] Run `npm --prefix frontend test` — must pass; record runtime delta.
- [X] T028 [US1] Run `npm --prefix frontend run i18n:check` — must pass; list any new keys introduced (or "None") in PR.
- [X] T029 [US1] Run `npm --prefix frontend run lint` — no new warnings; record in PR.
