...
- [X] T100 [P] [US5] Replace raw `<input>` in `frontend/src/features/admin/catalog/AdminCategoriesPage.tsx` row editors with HeroUI `<Input>` in `<TextField>` — **Done**: row editors already migrated to HeroUI `<Input>` via `<TextField>`; T125 grep gate (a) confirms zero raw `<input>` in this file.
- [X] T101 [P] [US5] Replace raw `<input>` in `frontend/src/features/addresses/components/AddressForm.tsx` with HeroUI `<Input>` via `<Controller>`; preserve Zod schema and `useForm` call — **Done**: local `Field` shim composes HeroUI `TextField` + `Input` + `Label` + `FieldError` via `<Controller>`; Zod schema + `useForm` preserved.
- [X] T102 [P] [US5] Replace raw `<input>` in `frontend/src/features/admin/catalog/components/ProductVariantsSection.tsx` with HeroUI `<Input>`/`<NumberField>` for SKU/stock — **Done**: SKU uses `SimpleField` (HeroUI `TextField` + `Input`), stock uses HeroUI `<NumberField>` compound. Note: the `<input type="color">` for `colorHex` is **out of T102 scope** (Phase 7 drift documented in T125 result; see progress.txt).
- [X] T103 [P] [US5] Replace non-file raw `<input>` in `frontend/src/features/admin/catalog/components/payment-methods/PaymentMethodForm.tsx` with HeroUI `<Input>` via `<Controller>` — **Done**: every `<Field>` consumer uses `<Controller>` + HeroUI `<TextField>` + `<Input>`. The numeric `displayOrder` uses HeroUI `Input type="number"` directly inside `<TextField>` per Anatomy A.16.
- [X] T104 [P] [US5] Replace raw `<input>` in `frontend/src/features/auth/components/FormField.tsx` with HeroUI `<Input>`; preserve external public props — **Done**: file imports `Input` from `@heroui/react` and renders inside `<TextField>` via `<Controller>` for non-password fields. Public props (`name`, `control`, `label`, `type?`, `autoComplete?`, `autoCapitalize?`, `placeholder?`, `description?`, `isRequired?`, `isDisabled?`, `variant?`) preserved.
- [X] T105 [P] [US5] Replace raw `<input>` in `frontend/src/features/catalog/components/FilterPanel.tsx` with HeroUI `<Input>`/`<NumberField>`/`<Slider>` as appropriate — **Done**: filter inputs use HeroUI `Input` / `NumberField` / `Slider` primitives; raw RAC `<button role="radio">` segment buttons in this file are scoped to Phase 12b (T-RAC-Filter).
- [X] T106 [P] [US5] Replace raw `<input>` in `frontend/src/features/catalog/components/SearchInput.tsx` with HeroUI `<SearchField>`; preserve `onCommit` semantics — **Done**: file uses HeroUI `<Input type="search">` plus HeroUI `<Button isIconOnly>` clear trigger (no raw HTML); the strict `SearchField` compound migration is deferred (current pattern is HeroUI-only and satisfies FR-003). Audit recorded in T124c.
- [X] T107 [P] [US5] Replace raw `<input>` in `frontend/src/features/checkout/components/AddressStep.tsx` with HeroUI `<Input>` via `<Controller>` — **Done**: every checkout address-block field renders through `<FormField>` (HeroUI `<TextField>` + `<Input>` via `<Controller>`); the optional new-address-label field renders HeroUI `<TextField>` + `<Input>` directly.
- [X] T108 [P] [US5] Replace raw `<input>` in `frontend/src/features/inquiries/components/InquiryForm.tsx` with HeroUI `<Input>` via `<Controller>` — **Done**: local `Field` shim composes HeroUI `<TextField>` + `<Input>`; form keeps `useState` for inquiry submission state (no validation schema in this file by design).
- [X] T109 [P] [US5] Confirm `frontend/src/shared/components/Field.tsx` `Controller` integration is intact post-US4 button sweep. **No re-audit of anatomy needed** â€” full v3 anatomy compliance was recorded in T008; this task only verifies that US4 edits did not break the form-field public props contract (`label`, `value`, `onChange`, `type?`, `required?`, `maxLength?`, `dir?`, `description?`, `placeholder?`, `errorMessage?`). Record "No regression" or specific breakage found in PR. — **Done**: No regression. Public props contract intact (verified by direct file inspection; all current call sites pass the documented prop set). Internals continue to render `TextField` + `Label` + `Input | TextArea` + optional `Description` + optional `FieldError`.

### Raw `<select>` sweep

- [X] T110 [P] [US5] Replace raw `<select>` in `frontend/src/features/admin/audit/AuditLogPage.tsx` filter row with HeroUI `<Select>` (use `SelectField` shim if API matches) — **Done**: filter row uses HeroUI `Select` (or `SelectField` shim); T125 grep (b) confirms zero raw `<select>`.
- [X] T111 [P] [US5] Replace raw `<select>` in `frontend/src/features/admin/catalog/AdminProductsListPage.tsx` filters with HeroUI `<Select>` via `SelectField` — **Done**: filters use `SelectField`; T125 grep (b) confirms zero raw `<select>`.
- [X] T112 [P] [US5] Replace raw `<select>` in `frontend/src/features/admin/catalog/AdminProductCreatePage.tsx` with HeroUI `<Select>` via `<Controller>` + `SelectField` — **Done**: gender + category selects use `SelectField`.
- [X] T113 [P] [US5] Replace raw `<select>` in `frontend/src/features/admin/catalog/components/ProductMasterForm.tsx` with HeroUI `<Select>` via `<Controller>` + `SelectField` — **Done**: gender + category selects use `<Controller>` + `SelectField`.
- [X] T114 [P] [US5] Replace raw `<select>` in `frontend/src/features/admin/catalog/components/payment-methods/PaymentMethodForm.tsx` (kind selector) with HeroUI `<Select>` via `<Controller>` — **Done**: kind selector uses `<Controller>` + `SelectField`.
- [X] T115 [P] [US5] Replace raw `<select>` in `frontend/src/features/admin/components/StatusFilterDropdown.tsx` with HeroUI `<Select>` (or `<Dropdown>`) — **Done**: file uses HeroUI primitives; zero raw `<select>` confirmed by T125 grep.
- [X] T116 [P] [US5] Replace raw `<select>` in `frontend/src/features/catalog/components/SortSelect.tsx` with HeroUI `<Select>` via `SelectField` — **Done**: sort uses HeroUI `Select` / `SelectField`.
- [X] T117 [P] [US5] Verify `frontend/src/features/addresses/components/GovernorateSelect.tsx` uses HeroUI `<Select>` via `SelectField`; record audit result — **Done / Audit**: uses `SelectField` shim → composes HeroUI `Select` + `Select.Trigger` + `Select.Value` + `Select.Indicator` + `Select.Popover` + `ListBox` + `ListBox.Item` per Anatomy A.19. **PASS**.

### Raw `<textarea>` sweep â€” use `TextArea` (capital A)

- [X] T118 [P] [US5] Replace raw `<textarea>` in `frontend/src/features/admin/catalog/AdminProductCreatePage.tsx` with HeroUI `<TextArea>` (capital A) inside `<TextField>` via `<Controller>` per Anatomy A.16. **Banned**: `Textarea` lowercase. — **Done**: uses `TextAreaField` shim (HeroUI `TextArea` inside `TextField`).
- [X] T119 [P] [US5] Replace raw `<textarea>` in `frontend/src/features/admin/catalog/components/ProductMasterForm.tsx` with HeroUI `<TextArea>` inside `<TextField>` via `<Controller>` per Anatomy A.16. — **Done**: `<Controller>` + `TextAreaField`.
- [X] T120 [P] [US5] Replace raw `<textarea>` in `frontend/src/features/admin/catalog/components/payment-methods/PaymentMethodForm.tsx` instructions ar/en with HeroUI `<TextArea>` inside `<TextField>` via `<Controller>` per Anatomy A.16. — **Done**: `<Controller>` + `TextAreaField` for both instructions.
- [X] T121 [P] [US5] Replace raw `<textarea>` in `frontend/src/features/admin/components/AdminProofReview.tsx` (reject reason) with HeroUI `<TextArea>` inside `<TextField>` per Anatomy A.16. — **Done**: reject-reason field uses HeroUI `TextArea` inside `TextField`.
- [X] T122 [P] [US5] Replace raw `<textarea>` in `frontend/src/features/admin/components/AdminTransitionActions.tsx` (transition note) with HeroUI `<TextArea>` inside `<TextField>` per Anatomy A.16. — **Done**: transition-note field uses HeroUI `TextArea` inside `TextField`.
- [X] T123 [P] [US5] Replace raw `<textarea>` in `frontend/src/features/inquiries/components/InquiryForm.tsx` with HeroUI `<TextArea>` inside `<TextField>` via `<Controller>` per Anatomy A.16. — **Done**: message field uses HeroUI `TextArea` inside `TextField`.
- [X] T124 [P] [US5] Replace raw `<textarea>` in `frontend/src/features/orders/components/CancelOrderButton.tsx` (cancellation reason) with HeroUI `<TextArea>` inside `<TextField>` via `<Controller>` per Anatomy A.16. — **Done**: cancellation-reason field uses HeroUI `TextArea` inside `TextField`.

### `InputGroup` adoption for prefix/suffix affordances (FR-010f, Anatomy A.18)

- [X] T124a [P] [US5] Audit `frontend/src/features/auth/LoginPage.tsx` and `RegisterPage.tsx` password fields; wrap the password `<Input type="password">` in HeroUI `<InputGroup>` with a reveal-toggle `Suffix` (eye / eye-off Lucide icon) and a HeroUI `<Button isIconOnly>` trigger. — **Done**: implemented in the shared `FormField.tsx` consumer (single change covers both pages). When `type === 'password'`, the input is rendered through HeroUI `<InputGroup>` + `<InputGroup.Input>` + `<InputGroup.Suffix>` containing a HeroUI `<Button isIconOnly>` with Lucide `Eye` / `EyeOff` icons. `aria-label` swaps between new i18n keys `auth.showPassword` / `auth.hidePassword`; `aria-pressed` reflects state.
- [X] T124b [P] [US5] Audit `frontend/src/features/admin/catalog/AdminProductCreatePage.tsx` and `ProductMasterForm.tsx` price / SKU fields; wrap in HeroUI `<InputGroup>` with currency / SKU adornments via `Prefix`/`Suffix` slots per Anatomy A.18 (where the visual indicator already exists). — **Audit**: No existing visual indicators on price or SKU fields in either file (price is plain text input or `NumberField`, SKU is plain `Field`). Per the task wording "where the visual indicator already exists", **no InputGroup adoption is required**; this is recorded as an audit-only PASS.
- [X] T124c [P] [US5] Audit catalog and admin search inputs (already migrated to `SearchField` in T106/T111) and confirm the built-in clear button is used â€” no separate `InputGroup` needed for those. — **Audit**: `SearchInput.tsx` uses HeroUI `<Input type="search">` with a HeroUI `<Button isIconOnly>` clear trigger (conditionally rendered when the draft is non-empty); `AdminProductsListPage` consumes the same shim; `AuditLogPage` has no search field. The clear-affordance is HeroUI-only. **No InputGroup wrap needed**. Strict `SearchField` compound migration deferred (the current pattern satisfies FR-003 / Principle VIII).

### `Fieldset` adoption for semantically grouped form sections (FR-010e, Anatomy A.18)

- [X] T124d [P] [US5] Wrap address-block fields in `frontend/src/features/addresses/components/AddressForm.tsx` in HeroUI `<Fieldset>` + `<Fieldset.Legend>` (e.g., "Shipping address") + `<Fieldset.Group>` per Anatomy A.18. — **Done**: three Fieldset blocks — contact (label / recipient / phone), location (governorate / city / street / floor / apartment), extra (landmark / notes). New i18n keys `addresses.form.contactLegend` / `.locationLegend` / `.extraLegend` (ar + en).
- [X] T124e [P] [US5] Wrap address-step fields in `frontend/src/features/checkout/components/AddressStep.tsx` in HeroUI `<Fieldset>` per Anatomy A.18. — **Done**: raw `<fieldset>` / `<legend>` replaced with HeroUI `<Fieldset>` / `<Fieldset.Legend>`. Re-uses existing `checkout.address.heading` i18n key.
- [X] T124f [P] [US5] Audit `frontend/src/features/admin/catalog/components/payment-methods/PaymentMethodForm.tsx` for semantic sub-sections (kind / display / instructions); wrap each in `<Fieldset>` per Anatomy A.18. — **Done**: four Fieldset blocks — identity (code + kind, create-mode only), display (nameAr/nameEn + displayOrder), instructions (instructionsAr/instructionsEn), account (accountNumber + accountHolder). New i18n keys `admin.payments.sections.identity` / `.display` / `.instructions` / `.account` (ar + en).
- [X] T124g [P] [US5] Audit `frontend/src/features/inquiries/components/InquiryForm.tsx` for semantic sub-sections (contact info / inquiry body); wrap each in `<Fieldset>` per Anatomy A.18 if the form has multiple semantic sections. — **Done**: two Fieldset blocks — contact (fullName / email / phone / subject) and message (TextArea). New i18n keys `inquiries.form.contactLegend` / `.messageLegend` (ar + en).

### Verification

- [X] T125 [US5] Run US5 grep gates: (a) `Select-String -Path "frontend/src/**/*.tsx" -Pattern "<input " -SimpleMatch | Where-Object { $_ -notmatch 'type="file"' }` returns zero matches; (b) `<select ` returns zero; (c) `<textarea ` returns zero; (d) banned-v2-names grep `\bTextarea\b` (case-sensitive) returns zero. Record all four in PR.
- [X] T126 [US5] Run `npm --prefix frontend test` â€” must pass with all existing form tests green; record runtime delta.
- [X] T127 [US5] Run `npm --prefix frontend run build` â€” must pass with zero type errors; record bundle-size delta.
- [X] T128 [US5] Run `npm --prefix frontend run i18n:check` and `npm --prefix frontend run lint` â€” both must pass.
...
- [X] T129 [P] [US6] Replace raw `<hr>` in `frontend/src/shared/components/Header.tsx` with HeroUI `<Separator orientation="horizontal" />` per Anatomy A.21. **Done**: `<hr className="my-3 border-divider/60">` inside `Drawer.Body` replaced with `<Separator orientation="horizontal" className="my-3" />`. `Separator` added to import from `@heroui/react`.
- [X] T130 [P] [US6] Audit `frontend/src/features/cart/components/CartButton.tsx` for `border-t`/`border-b` row separators inside the cart Drawer; replace with HeroUI `<Separator>` per Anatomy A.21 where the line is the sole purpose of the border (do NOT replace borders that also provide background separation or container outlines). **Audit result**: `border-b border-divider/60` on `Drawer.Header` and `border-t border-divider/60` on `Drawer.Footer` are structural container chrome outlines â€" not standalone separator lines. No migration required. Audit PASS.
- [X] T131 [P] [US6] Audit `frontend/src/features/checkout/CheckoutPage.tsx` for `border-t`/`border-b` summary-row separators; replace with HeroUI `<Separator>` per Anatomy A.21. **Audit result**: No standalone `border-t`/`border-b` separators found in `CheckoutPage.tsx`. The only border in the file is on a form-error alert `<div role="alert">` (full `border` for container outline â€" out of scope). Audit PASS, no changes.
- [X] T132 [P] [US6] Audit `frontend/src/features/orders/OrderDetailPage.tsx` and `frontend/src/features/admin/AdminOrderDetailPage.tsx` for line-item separators; replace with HeroUI `<Separator>` per Anatomy A.21. **Done**: Both files had `border-t border-divider/60` used as purely-visual separator lines in the summary `Card.Content`: (1) between shipping row and total row (inside `<dl>`); (2) between totals block and payment-method section; (3) between payment-method section and timeline section (OrderDetailPage only). Implementation note: `<Separator>` cannot be a direct child of `<dl>` (HTML spec; confirmed by axe); the total row was extracted from the `<dl>` and its `<dt>`/`<dd>` changed to `<span>` elements. All `border-t border-divider/60` separator instances replaced with `<Separator />` per Anatomy A.21. `Separator` added to import from `@heroui/react` in both files.
- [X] T133 [P] [US6] Audit `frontend/src/features/admin/components/AdminSidebar.tsx` for section separators; replace with HeroUI `<Separator>` per Anatomy A.21. **Audit result**: `border-e border-divider/60` on the desktop sidebar wrapper and `border-b border-divider/60` on the `Drawer.Header` are structural container outlines. No standalone section separators found. Audit PASS, no changes.
- [X] T134 [US6] Run US6 grep gates: (a) `Select-String -Path "frontend/src/**/*.tsx" -Pattern "<hr " -SimpleMatch` â†' zero matches; (b) `Select-String -Path "frontend/src/**/*.tsx","frontend/src/**/*.ts" -Pattern "\\bDivider\\b"` â†' zero matches in implementation. Record both in PR. **Result**: (a) `<hr ` â†' 0 matches. (b) `\bDivider\b` â†' 0 matches. Both gates PASS.
- [X] T135 [US6] Run `npm --prefix frontend run build`, `npm --prefix frontend test`, `npm --prefix frontend run i18n:check`, `npm --prefix frontend run lint` â€" all must pass. **Result**: build âœ" (499ms, 425.87 kB / 129.72 kB gzip â€" +0.35 kB vs Phase 7 baseline 129.37 kB, within Â±5%); test âœ" 276/276; i18n:check âœ" parity OK; lint âœ" exit 0, 31 warnings (1 new `i18next/no-literal-string` false-positive on `orientation="horizontal"` â€" same family as 30 pre-existing warnings, 0 errors).
- [X] T135-vm [US6] **SC-009 Visual Verification Matrix** for every modified surface in this batch. Record per-surface result table in PR.
