...
- [X] T035 [US2] Run US2 grep gates: (a) `Select-String -Path "frontend/src/**/*.tsx" -Pattern "<table " -SimpleMatch` → zero matches; (b) `Select-String -Path "frontend/src/**/*.tsx" -Pattern "\\b(TableHeader|TableColumn|TableBody|TableRow|TableCell)\\b"` → zero matches in modified files (only dot-notation `Table.*`). Record both in PR.
- [X] T036 [US2] Run `npm --prefix frontend run build` — must pass; record bundle-size delta.
- [X] T037 [US2] Run `npm --prefix frontend test` — must pass (existing admin list tests cover this); record runtime delta.
- [X] T038 [US2] Run `npm --prefix frontend run i18n:check` and `npm --prefix frontend run lint` — both must pass.
