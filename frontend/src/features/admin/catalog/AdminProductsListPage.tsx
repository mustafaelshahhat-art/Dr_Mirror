...
/**
 * Admin product queue at <c>/admin/products</c>. Filter by category / gender /
 * publish state / text. Clicking a row goes to the edit page.
 */
export function AdminProductsListPage() {
  ...
  const products = useAdminProductsQuery({ q: q || undefined, categoryId, gender, published });
    ...
    <section className="space-y-5">
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_auto_auto_auto]">
        <input
          ...
          className="rounded-medium border border-divider bg-content1 px-3 py-1.5 text-sm"
        />
        <select
          value={categoryId ?? ''}
